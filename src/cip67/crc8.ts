const table_src = Object.freeze([
    
] as const); 

const TABLE = typeof Uint8Array !== "undefined" ? new Uint8Array( table_src ) : table_src;

type crc8_checksum = typeof table_src[number]

export function crc8(
    current: Uint8Array,
    previous: number = 0,
): crc8_checksum
{
    // cast to singed int 32 (we only care that it is 32 bits)
    let crc = Number( previous ) | 0 ;

    if(
        typeof Uint8Array !== "undefined" 
    ) {
        if( current instanceof Uint8Array ) {} // all good
        else if(
            Array.isArray( current ) && 
            (current as any[]).every( v => typeof v === "number" )
        )
        {
            current = new Uint8Array( current );
        }
        else throw new TypeError("unexpected value for crc8 checksum");
    }
    else if( 
        Array.isArray( current ) && 
        (current as any[]).every( v => typeof v === "number" )
    )
    {
        current = current.map( v => v & 0xff );
    }
    else throw new TypeError("unexpected value for crc8 checksum");

  
    for (let index = 0; index < current.length; index++) {
      crc = TABLE[(crc ^ current[index]) & 0xff];
    }
  
    return crc as crc8_checksum;
}